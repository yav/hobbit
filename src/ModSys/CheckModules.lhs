...
> module ModSys.CheckModules(ModSysErr(..),chkModule) where
>
> import ModSys.Relations
> import ModSys.NamesEntities
> import ModSys.AST
...
considering value constructors and other entities separately.
...
>       Just (Subs subs) -> 
>         map (errUndefSub x)
>             (filter (not . (`Set.member` subsInScope)) subs)
>         where
>         subsInScope = 
>           Set.map toSimple 
>             $ dom 
>             $ restrictRng (`Set.member` owns ent) rel
>       _ -> []
>
>   consider
